...
# Coletando os dados do ativo
dataini <- as.Date("2025-04-01")
datafim <- as.Date("2025-06-28")
ativo <- get.hist.quote("lwsa3.sa", quote = "Close", start = dataini, end = datafim)
ativo <- na.omit(ativo)
...
# Usando função de suavização (opcional)
precos_suavizados <- SMA(precos, n = 3)

# Detectar mudanças de direção simples
fundos <- c()
topos <- c()

for (i in 2:(length(precos_suavizados) - 1)) {
  if (!is.na(precos_suavizados[i])) {
    if (precos_suavizados[i] < precos_suavizados[i - 1] && precos_suavizados[i] < precos_suavizados[i + 1]) {
      fundos <- c(fundos, i)
    } else if (precos_suavizados[i] > precos_suavizados[i - 1] && precos_suavizados[i] > precos_suavizados[i + 1]) {
      topos <- c(topos, i)
    }
  }
}
...
# Plotar o gráfico
plot(ativo, main = "Análise de Dow - LWSA3", col = "black")

# Marcar topos e fundos
points(datas[fundos], precos[fundos], col = "blue", pch = 17, cex = 1.2)  # Fundos
points(datas[topos], precos[topos], col = "red", pch = 19, cex = 1.2)     # Topos

# Traçar linhas conectando topos e fundos para indicar tendência
